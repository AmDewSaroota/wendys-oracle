...
Create a JSON file following the `SlideDeck` format. Write it to `skills/slide/tmp/slides-<timestamp>.json`.
...
```bash
cd skills/slide && npx tsx scripts/preview-html.ts tmp/slides-<timestamp>.json
```
...
```bash
cd skills/slide && npx tsx scripts/create-slides.ts tmp/slides-<timestamp>.json
```
...
```bash
cd skills/slide && npm install && npx tsx scripts/setup.ts
```
